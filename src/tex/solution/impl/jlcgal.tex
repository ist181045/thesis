...
\begin{quote}
  ...
  appropriate call with \mintinline{julia}{ccall} syntax, which looks like an
...
line 4 where we preceded our wrapper function with \mintinline{cpp}{extern "C"}.
...
alternative to \mintinline{julia}{ccall} by way of the slightly more
convenient\footnote{Your mileage may vary.} \mintinline{julia}{@ccall} macro.
...
overcome this limitation by mirroring C \mintinline{c}{struct}s in Julia.  As an
...
We could try and directly mirror \ac{CGAL}'s \texttt{Point\_3} type, but that
would require that we knew its layout.  To circumvent this completely, we can
just create a \mintinline{c}{struct} that opaquely wraps around \ac{CGAL}'s
...
\todo{Even then I don't think this is possible.  Must confirm.}
...
the C++ wrapper code depends on.  \Cref{lst:solution.impl.jlcgal.jlcxx}
...
\mintinline{c}{extern "C"}, but slightly more robust.}
