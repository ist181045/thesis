...
\textit{Geometric Constraint Primitives} is loosely inspired on tools like
\texttt{tkz-euclide} and Eukleides.  Both are based on Euclidean geometry, a
constructive system where the production of geometry can be done solely
resorting to a straightedge and a compass.  This makes programs described using
these constructs easier to understand and manually reproduce.
...
that way, defining a \texttt{circumcenter} function similar to the following
\begin{minted}{julia}
circumcenter(a, b, c) = intersection(bisector(a, b), bisector(b, c)) 
\end{minted}
However, this is one of those occasions where this functionality is already
present in \ac{CGAL}.  This is a simple yet perfect demonstration of one of our
approach's benefits w.r.t. repurposing a comprehensive library with plenty of
...
that of tangencies.  Generally speaking, there are no constructs that specific
...
\begin{listing}[htbp]
  ...
  \caption[Circle tangent to a line]{
    Implementation of the ``Circle tangent to a line'' problem.  Note this works
    with \texttt{infinite} lines, not necessarily line \texttt{segments}.}%
...
We did not implement such a scenario regarding line segments, for we did not
consider it, keeping with the implementation in
\cref{lst:solution.impl.gcps.tangcircle2line}.  A potential solution to that
specific problem could be obtained reusing the \texttt{tangent\_circle}
function, however.  We would then need to verify if the circle would fall
somewhere between the segment's endpoints.  Otherwise, there would not be a
solution available.
