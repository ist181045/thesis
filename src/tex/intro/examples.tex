...
\begin{figure}[htbp]
  ...
  \subfloat[Line that goes through $C$, strictly parallel to
    ...
    \resizebox{.4\textwidth}{!}{\input{tikz/ex-parallel}}}%
  \hspace{\fill}%
  \subfloat[$\odot O_r$ circumscribed about $\triangle ABC$]{%
    \label{fig:intro.example.circumcenter}%
    \resizebox{.4\textwidth}{!}{\input{tikz/ex-circumcenter}}}%
...
be further simplified for a $2\times 2$ matrix. Let $B\in\mathbb{R}^{2\times 2}$
be the matrix whose columns are the vectors $\vec{AB} = (a, b)$ and $\vec{AC} =
(c, d)$, and $i' = 1$, for instance, such that, from
