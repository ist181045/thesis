...
\ac{TikZ}~\cite{Tantau:2015:tikz-manual} and Eukleides~\cite{Obrecht:2010:EM}.
...
\begin{figure}[htpb]
  \subcaptionbox{\label{fig:intro.example.parallel}%
    ...
    [.45\linewidth]{\resizebox{!}{.2\textheight}{\input{tikz/ex-parallel}}}
  \hspace{\fill}
  \subcaptionbox{\label{fig:intro.example.circumcenter}%
    $\odot O_r$ circumscribed about $\triangle ABC$.}
    [.45\linewidth]{\resizebox{!}{.2\textheight}{\input{tikz/ex-circumcenter}}}
...
$\vec{u_1},\,\vec{u_2},\,\vec{u_3}$ the edges' normal vectors, such that
\[
  \begin{split}
